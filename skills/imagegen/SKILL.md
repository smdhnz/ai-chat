...
- **Default built-in tool mode (preferred):** pi `imagegen` tool for image generation, editing, and transparent-image requests. Does not require `OPENAI_API_KEY`.
...
- For transparent images, ask pi `imagegen` for a transparent background and preserve the generated alpha.
- Never silently switch from pi `imagegen` or CLI `gpt-image-2` to CLI `gpt-image-1.5`; ask the user first unless they explicitly requested `gpt-image-1.5`.
...
- In built-in tool mode, Codex saves generated images under `~/.pi/agent/generated_images/*` by default.
- Do not describe or rely on OS temp as the default built-in destination.
- The pi `imagegen` tool supports `outputPath` as a pi-specific addition. It always writes PNG data and appends `.png` when needed.
- Save-path precedence in built-in mode:
  1. If the user names a destination, pass it as `outputPath`.
  2. If the image is meant for the current project, pass a workspace `outputPath` before finishing.
  3. If the image is only for preview or brainstorming, omit `outputPath`; the file can remain at the default `~/.pi/agent/generated_images/*` path.
- Never leave a project-referenced asset only at the default `~/.pi/agent/generated_images/*` path.
...
1. If another project skill supplies domain-specific art direction (for example `visual-art-direction`, `illustration-art-direction`, or `nail-design`), use it to shape the brief while keeping this skill's tool and save-path rules authoritative.
2. Decide the top-level mode: built-in by default, including transparent-output requests; fallback CLI only if explicitly requested or confirmed.
...
8. For edits or visual references, pass up to five local paths through `referenced_image_paths`. Inspect unfamiliar inputs first when an image-reading tool is available.
...
11. Use the pi `imagegen` tool by default.
12. For transparent-output requests, ask pi `imagegen` for a transparent background and preserve the generated alpha channel.
13. Inspect outputs and validate: subject, style, composition, text accuracy, and invariants/avoid items.
14. Iterate with a single targeted change, then re-check.
15. For preview-only work, render the image inline; the underlying file may remain at the default `~/.pi/agent/generated_images/...` path.
16. For project-bound work, move or copy the selected artifact into the workspace and update any consuming code or references. Never leave a project-referenced asset only at the default `~/.pi/agent/generated_images/...` path.
17. For batches or multi-asset requests, persist every requested deliverable final in the workspace unless the user explicitly asked to keep outputs preview-only. Discarded variants do not need to be kept unless requested.
18. If the user explicitly chooses or confirms the CLI fallback, then use the fallback-only docs for model, quality, size, `input_fidelity`, masks, output format, output paths, and network setup.
19. Always report the final saved path(s) for any workspace-bound asset(s), plus the final prompt or prompt set and whether the built-in tool or fallback CLI mode was used.

## Transparent image requests

Ask pi `imagegen` for a genuinely transparent background and preserve its alpha.
...
- background-extraction — transparent background / clean cutout. Ask pi `imagegen` for actual transparency.
...
- Fallback-only execution notes such as `Quality:`, `Input fidelity:`, masks, output format, and output paths belong in the CLI path only. Do not treat them as pi `imagegen` tool arguments.
...
- For transparent images, ask pi `imagegen` for actual transparency and preserve its alpha.
...
These conventions apply only to the CLI fallback. They do not describe pi `imagegen` output behavior.
...
- Do not ask the user for `OPENAI_API_KEY` when using the pi `imagegen` tool.
