...
- Do not silently downgrade from CLI `gpt-image-2` or pi `imagegen` to CLI `gpt-image-1.5`; ask first unless the user explicitly requested `gpt-image-1.5`.
...
These are explicit CLI controls. They are not pi `imagegen` tool arguments.
