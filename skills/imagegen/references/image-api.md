...
These parameters describe the Image API and bundled CLI fallback surface. Do not assume they are normal arguments on the pi `imagegen` tool.

## Scope

- This fallback CLI is intended for GPT Image models (`gpt-image-2`, `gpt-image-1.5`, `gpt-image-1`, and `gpt-image-1-mini`).
- The pi `imagegen` tool and the fallback CLI do not expose the same controls.
...
`gpt-image-2` does not currently support the Image API `background=transparent` parameter. In explicit CLI/API fallback mode, keep `gpt-image-2` when a flat chroma-key background plus local alpha extraction with `python "$HOME/.pi/agent/skills/imagegen/scripts/remove_chroma_key.py"` is acceptable.
...
- Do not assume they are pi `imagegen` tool arguments.
