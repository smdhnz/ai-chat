...
管理者だけ設定画面に「管理者モード」のオン／オフが表示されます。通常はオフで、オンにすると通常のサイドバーに全ユーザーのチャットが所有者付きで表示されます（プロジェクト内・保存中の一時チャットも対象）。選択すると通常のチャット画面で会話本文と添付・生成画像を閲覧できます。他ユーザーの会話では入力・編集・再生成・削除を表示せず、読み取り専用になります。自分の会話は通常どおり操作できます。オフにするかページを再読み込みすると自分が通常アクセスできる一覧に戻り、取得済みの管理者表示データは保持しません。他ユーザーの本文は埋め込みURLへの自動アクセスを避けるためプレーンテキスト表示です。
