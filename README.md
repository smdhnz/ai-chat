...
管理者だけ設定画面に「管理者モード」のオン／オフが表示されます。初期状態はオフで、トグルの状態はDiscordユーザー別に同じブラウザーへ保存します。非管理者には設定項目を表示しません。オンにすると通常のサイドバーに全ユーザーのチャットが所有者の表示名付きで表示されます（保存中の一時チャットも対象）。プロジェクト内の会話は所属プロジェクトの一覧に表示し、未参加のプロジェクトも閲覧用として選択できます。選択すると通常のチャット画面で会話本文と添付・生成画像を閲覧できます。他ユーザーの会話では入力・編集・再生成・削除を表示せず、読み取り専用になります。自分の会話は通常どおり操作できます。オフにすると自分が通常アクセスできる一覧に戻り、取得済みの管理者表示データは保持しません。再読み込み時は保存したモードを復元し、会話・画像は権限を再検証して取得します（本文・画像はブラウザーストレージに保存しません）。他ユーザーの本文は埋め込みURLへの自動アクセスを避けるためプレーンテキスト表示です。
