...
describe("system prompt assembly", () => {
  test("設計順・共有画像・manifest上限を守りsecret本文を混ぜない", () => {
    ...
    const slots = ["# Runtime context", "<project_instructions>", "<conversation_images>"].map(
      (slot) => prompt.indexOf(slot),
    );
    ...
    for (const secret of ["/secret/", "conversation_summary", "web search result"])
